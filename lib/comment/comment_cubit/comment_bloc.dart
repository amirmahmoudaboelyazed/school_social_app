...
class CommentBloc extends Bloc<CommentEvent, CommentState> {
  ...
  Future<void> _onCommentFetched(
    ...
    try {
      if (state.status == CommentStatus.initial) {
        final comment = await _fetchComment(currentPage: 0,pageSize: 5,id: event.id);
      ...
      final comment = await _fetchComment(currentPage: currentPage,pageSize: 5);
      comment.isEmpty
  ...
  Future<List<CommentModel>> _fetchComment({int? currentPage,int? pageSize,int? id}) async {
    List<CommentModel> comment =[];
    Response? response = await DioMethods.getComment(currentPage: currentPage!, pageSize: pageSize!, postId: id!);
